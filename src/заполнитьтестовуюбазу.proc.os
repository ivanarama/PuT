...
Процедура Выполнить()
  ...
  Если Очищаем Тогда
    Удал = 0;
    Попытка
      // 1. Снимаем проведение со всех документов для снятия блокировок и движений
      ЗД = Новый Запрос; ЗД.Текст = "ВЫБРАТЬ Номер ИЗ Документ.ВозвратОтПокупателя";
      Номера = Новый Массив;
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ВозвратОтПокупателя.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
              ДокОб.ОтменитьПроведение();
            КонецЕсли;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ВозвратПоставщику.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
        ...
        КонецЕсли;
      КонецЦикла;
      ЗД = Новый Запрос; ЗД.Текст = "ВЫБРАТЬ Номер ИЗ Документ.КорректировкаДолга";
      Номера = Новый Массив;
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.КорректировкаДолга.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
        ...
        КонецЕсли;
      КонецЦикла;
      ЗД = Новый Запрос; ЗД.Текст = "ВЫБРАТЬ Номер ИЗ Документ.ЗакрытиеЗаказовПокупателей";
      Номера = Новый Массив;
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ЗакрытиеЗаказовПокупателей.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
        ...
        КонецЕсли;
      КонецЦикла;
      ЗД = Новый Запрос; ЗД.Текст = "ВЫБРАТЬ Номер ИЗ Документ.ЗакрытиеЗаказовПоставщикам";
      Номера = Новый Массив;
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ЗакрытиеЗаказовПоставщикам.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ПриходныйКассовыйОрдер.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.РасходныйКассовыйОрдер.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ПоступлениеНаРасчётныйСчёт.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.СписаниеСРасчётногоСчёта.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.РеализацияТоваров.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.СписаниеТоваров.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ПеремещениеТоваров.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.СборкаТоваров.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.РазборкаТоваров.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ПоступлениеТоваров.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ОприходованиеТоваров.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ЗаказПоставщику.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.ЗаказПокупателя.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.Счёт.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
      ...
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        Сс = Документы.УстановкаЦен.НайтиПоНомеру(Номер);
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            ДокОб = Сс.ПолучитьОбъект();
            Если ЗначениеЗаполнено(ДокОб) Тогда
        ...
        КонецЕсли;
      КонецЦикла;
      // 2. Удаляем все документы в порядке зависимости ссылок
      ЗД = Новый Запрос; ЗД.Текст = "ВЫБРАТЬ Номер ИЗ Документ.ВозвратОтПокупателя";
      Номера = Новый Массив;
      Для Каждого Стр Из ЗД.Выполнить() Цикл Номера.Добавить(Стр.Номер); КонецЦикла;
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ВозвратОтПокупателя.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ВозвратПоставщику.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.КорректировкаДолга.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ЗакрытиеЗаказовПокупателей.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ЗакрытиеЗаказовПоставщикам.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ПриходныйКассовыйОрдер.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.РасходныйКассовыйОрдер.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ПоступлениеНаРасчётныйСчёт.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.СписаниеСРасчётногоСчёта.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.РеализацияТоваров.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.СписаниеТоваров.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ПеремещениеТоваров.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.СборкаТоваров.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.РазборкаТоваров.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ПоступлениеТоваров.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ОприходованиеТоваров.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ЗаказПоставщику.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.ЗаказПокупателя.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
      ...
      Для Каждого Номер Из Номера Цикл
        ...
        Если ЗначениеЗаполнено(Сс) Тогда
          Попытка
            Документы.УстановкаЦен.Удалить(Сс);
            Удал = Удал + 1;
          Исключение
          КонецПопытки;
        КонецЕсли;
  ...
  Если Создавать Тогда
    ...
    ВозвПост1.Склад = СкладГлавный; ВозвПост1.Основание = Пост1.Ссылка; ВозвПост1.СпособУчетаНДС = "ВТомЧисле";
