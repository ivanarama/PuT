...
Процедура Выполнить()
  ...
  Если Создавать Тогда
    ...
    // Розничные цены (с наценкой ~30%)
    УЦР = Документы.УстановкаЦен.Создать();
    УЦР.Номер = "УЦ-00002";
    УЦР.Дата = ДатаМ1; УЦР.Организация = НашаОрг; УЦР.ТипЦен = ТипРозн;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомТабурет;  Стр.Цена = 900;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомКресло;   Стр.Цена = 5500;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомСтол;     Стр.Цена = 4500;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомДиван;    Стр.Цена = 14000;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомПолка;    Стр.Цена = 1200;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомПуф;      Стр.Цена = 750;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомШкаф;     Стр.Цена = 22000;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомКомод;    Стр.Цена = 9000;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомТумбочка; Стр.Цена = 2800;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомКровать;  Стр.Цена = 25000;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомСтеллаж;  Стр.Цена = 6500;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомБанкетка; Стр.Цена = 1100;
    Стр = УЦР.Цены.Добавить(); Стр.Номенклатура = НомНабор;    Стр.Цена = 6300;
    УЦР.Записать(); УЦР.Провести();

    Сообщить("Установка цен (оптовые + розничные, 13+13 позиций) проведена.");
