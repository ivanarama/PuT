...
Процедура Выполнить()
  ...
  Если Создавать Тогда
    ...
    // Разносим даты «по этажам»: сначала приход товара, потом расход — чтобы
    // данные были хронологически достоверны (поступление раньше реализации) и
    // отчёты «на дату» считались верно. ФИФО при перепроведении теперь от дат
    // не зависит — остатки берутся на МоментВремени документа (П.1), — но
    // осмысленные даты всё равно нужны.
    // Сдвиг — только помесячно: «дата − дни» платформа не считает (П.35),
    // есть только ДобавитьМесяц.
    ДатаУставной = ДобавитьМесяц(Сегодня, -5); // взнос учредителя ~начало года
    ДатаЗаказовПост = ДобавитьМесяц(Сегодня, -4); // заказы поставщикам
    ДатаПрихода  = ДобавитьМесяц(Сегодня, -3);  // поступления, оприходования
    ДатаСклада   = ДобавитьМесяц(Сегодня, -2);  // перемещения, сборки
    ДатаПродаж   = Сегодня;                     // реализации, списания, счета, заказы покупателей
    ДатаДенег    = Сегодня;                     // ПКО/РКО и банк — после продаж

    // ---- ВЗНОС УЧРЕДИТЕЛЯ ----

    ПКОуст = Документы.ПриходныйКассовыйОрдер.Создать();
    ПКОуст.Номер = "ПКО-00000";
    ПКОуст.Дата = ДатаУставной; ПКОуст.Организация = НашаОрг; ПКОуст.Касса = КассаНал;
    ...
    // ---- УСТАНОВКА ЦЕН (оптовые) ----

    УЦ = Документы.УстановкаЦен.Создать();
    УЦ.Номер = "УЦ-00001";
    УЦ.Дата = ДатаПрихода; УЦ.Организация = НашаОрг; УЦ.ТипЦен = ТипОпт;
    ...
    Сообщить("Установка цен (оптовые, 13 позиций) проведена.");

    // ---- ЗАКАЗЫ ПОСТАВЩИКАМ ----

    ЗакПост1 = Документы.ЗаказПоставщику.Создать();
    ЗакПост1.Номер = "ЗПС-00001";
    ЗакПост1.Дата = ДатаЗаказовПост; ЗакПост1.Организация = НашаОрг; ЗакПост1.Поставщик = ПоставщикДерево;
    ...
    ЗакПост2.Дата = ДатаЗаказовПост; ЗакПост2.Организация = НашаОрг; ЗакПост2.Поставщик = ПоставщикФанера;
    ...
    Пост1.Дата = ДатаПрихода; Пост1.Организация = НашаОрг; Пост1.Поставщик = ПоставщикДерево;
    ...
    Пост2.Дата = ДатаПрихода; Пост2.Организация = НашаОрг; Пост2.Поставщик = ПоставщикФанера;
    ...
    Пост3.Дата = ДатаПрихода; Пост3.Организация = НашаОрг; Пост3.Поставщик = ПоставщикГвоздь;
    ...
    Пост4.Дата = ДатаПрихода; Пост4.Организация = НашаОрг; Пост4.Поставщик = ПоставщикБезРучек;
    ...
    Опр1.Дата = ДатаПрихода; Опр1.Организация = НашаОрг; Опр1.Склад = СкладУценка;
    ...
    Опр2.Дата = ДатаПрихода; Опр2.Организация = НашаОрг; Опр2.Склад = СкладУценка;
    ...
    Пер1.Дата = ДатаСклада; Пер1.Организация = НашаОрг;
    ...
    Пер2.Дата = ДатаСклада; Пер2.Организация = НашаОрг;
    ...
    // Собираем 1 «Набор Столовый» из 2 табуретов + 1 стола
    Сбр = Документы.СборкаТоваров.Создать();
    Сбр.Номер = "СБР-00001";
    Сбр.Дата = ДатаСклада; Сбр.Организация = НашаОрг; Сбр.Склад = СкладГлавный;
    ...
    // Счёт 1: Кафе «Три Стула»
    Сч1 = Документы.Счёт.Создать();
    Сч1.Номер = "СЧ-00001";
    Сч1.Дата = ДатаПродаж; Сч1.Организация = НашаОрг; Сч1.Покупатель = ПокупательКафе;
    Сч1.Склад = СкладГлавный; Сч1.ТипЦенПродажи = ТипРозн; Сч1.СпособУчетаНДС = "ВТомЧисле";
    Сч1.СрокОплаты = ДобавитьМесяц(ДатаПродаж, 1);
    ...
    Сч2.Номер = "СЧ-00002";
    Сч2.Дата = ДатаПродаж; Сч2.Организация = НашаОрг; Сч2.Покупатель = ПокупательСядь;
    Сч2.Склад = СкладГлавный; Сч2.ТипЦенПродажи = ТипРозн; Сч2.СпособУчетаНДС = "ВТомЧисле";
    Сч2.СрокОплаты = ДобавитьМесяц(ДатаПродаж, 1);
    Сч2.Комментарий = "Диваны и полки —躺 в любой позе";
    ...
    // Счёт 3: Гостиница «Три подушки»
    Сч3 = Документы.Счёт.Создать();
    Сч3.Номер = "СЧ-00003";
    Сч3.Дата = ДатаПродаж; Сч3.Организация = НашаОрг; Сч3.Покупатель = ПокупательГостиница;
    Сч3.Склад = СкладГлавный; Сч3.ТипЦенПродажи = ТипРозн; Сч3.СпособУчетаНДС = "ВТомЧисле";
    Сч3.СрокОплаты = ДобавитьМесяц(ДатаПродаж, 1);
    ...
    Сч4.Номер = "СЧ-00004";
    Сч4.Дата = ДатаПродаж; Сч4.Организация = НашаОрг; Сч4.Покупатель = ПокупательРемонт;
    Сч4.Склад = СкладГлавный; Сч4.ТипЦенПродажи = ТипРозн; Сч4.СпособУчетаНДС = "ВТомЧисле";
    Сч4.СрокОплаты = ДобавитьМесяц(ДатаПродаж, 1);
    ...
    // Заказ 2: Гостиница «Три подушки» — по счёту СЧ-00003
    ЗакПок2 = Документы.ЗаказПокупателя.Создать();
    ЗакПок2.Номер = "ЗПК-00002";
    ЗакПок2.Дата = ДатаПродаж; ЗакПок2.Организация = НашаОрг; ЗакПок2.Покупатель = ПокупательГостиница;
    ЗакПок2.Склад = СкладГлавный; ЗакПок2.ТипЦенПродажи = ТипРозн; ЗакПок2.СпособУчетаНДС = "ВТомЧисле";
    ЗакПок2.Основание = Сч3;
    ЗакПок2.Комментарий = "Комплектация номеров — кровати, тумбы, шкаф";
    Стр = ЗакПок2.Товары.Добавить(); Стр.Номенклатура = НомКровать;  Стр.Количество = 2; Стр.Цена = 27000;
    ...
    Сообщить("Заказы покупателей (2 шт.) проведены.");
    ...
    Реал1.Дата = ДатаПродаж; Реал1.Организация = НашаОрг; Реал1.Покупатель = ПокупательКафе;
    ...
    Реал2.Дата = ДатаПродаж; Реал2.Организация = НашаОрг; Реал2.Покупатель = ПокупательСядь;
    ...
    Реал3.Дата = ДатаПродаж; Реал3.Организация = НашаОрг; Реал3.Покупатель = ПокупательРесторан;
    ...
    Реал4.Дата = ДатаПродаж; Реал4.Организация = НашаОрг; Реал4.Покупатель = ПокупательДетсад;
    ...
    Реал5.Дата = ДатаПродаж; Реал5.Организация = НашаОрг; Реал5.Покупатель = ПокупательГостиница;
    ...
    Реал6.Дата = ДатаПродаж; Реал6.Организация = НашаОрг; Реал6.Покупатель = ПокупательРемонт;
    ...
    // Возврат 1: Кафе «Три Стула» возвращает 3 табурета из Реал1 — слишком шаткие
    ВозвПок1 = Документы.ВозвратОтПокупателя.Создать();
    ВозвПок1.Номер = "ВОЗ-00001";
    ВозвПок1.Дата = ДатаПродаж; ВозвПок1.Организация = НашаОрг; ВозвПок1.Покупатель = ПокупательКафе;
    ВозвПок1.Склад = СкладГлавный; ВозвПок1.Основание = Реал1; ВозвПок1.СпособУчетаНДС = "ВТомЧисле";
    ...
    Сообщить("Возвраты от покупателей (1 шт.) проведены.");

    // ---- ВОЗВРАТЫ ПОСТАВЩИКАМ ----

    // Возврат 1: Дерево и Гвозди — 5 бракованных табуретов из Пост1 (три ножки вместо четырёх)
    ВозвПост1 = Документы.ВозвратПоставщику.Создать();
    ВозвПост1.Номер = "ВПС-00001";
    ВозвПост1.Дата = ДатаПродаж; ВозвПост1.Организация = НашаОрг; ВозвПост1.Поставщик = ПоставщикДерево;
    ВозвПост1.Склад = СкладГлавный; ВозвПост1.Основание = Пост1; ВозвПост1.СпособУчетаНДС = "ВТомЧисле";
    ...
    Сообщить("Возвраты поставщикам (1 шт.) проведены.");
    ...
    ПКО1.Дата = ДатаДенег; ПКО1.Организация = НашаОрг; ПКО1.Касса = КассаНал;
    ...
    ПКО2.Дата = ДатаДенег; ПКО2.Организация = НашаОрг; ПКО2.Касса = КассаНал;
    ...
    БнкП1.Дата = ДатаДенег; БнкП1.Организация = НашаОрг; БнкП1.БанковскийСчёт = СчётБанк;
    ...
    БнкП2.Дата = ДатаДенег; БнкП2.Организация = НашаОрг; БнкП2.БанковскийСчёт = СчётБанк;
    ...
    РКО1.Дата = ДатаДенег; РКО1.Организация = НашаОрг; РКО1.Касса = КассаНал;
    ...
    РКО2.Дата = ДатаДенег; РКО2.Организация = НашаОрг; РКО2.Касса = КассаНал;
    ...
    БнкР1.Дата = ДатаДенег; БнкР1.Организация = НашаОрг; БнкР1.БанковскийСчёт = СчётБанк;
    ...
    БнкР2.Дата = ДатаДенег; БнкР2.Организация = НашаОрг; БнкР2.БанковскийСчёт = СчётБанк;
    ...
    Спис1.Дата = ДатаПродаж; Спис1.Организация = НашаОрг; Спис1.Склад = СкладГлавный;
    ...
    Спис2.Дата = ДатаПродаж; Спис2.Организация = НашаОрг; Спис2.Склад = СкладГлавный;
    ...
    // Разбираем 1 диван — получаем 3 пуфа и 2 полки (диван уже не тот...)
    Рзб = Документы.РазборкаТоваров.Создать();
    Рзб.Номер = "РЗБ-00001";
    Рзб.Дата = ДатаПродаж; Рзб.Организация = НашаОрг; Рзб.Склад = СкладГлавный;
