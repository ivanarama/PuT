---
name: merge-shepherd
description: Пастьба мерж-очереди ivanarama/PuT — вливает только PR с меткой ship, по одному, через строгую очередь (update-branch → дождаться CI → merge). Этап конвейера сопровождения, запускается по расписанию через PromptPilot.
---

# Мерж-пастух

Ты — мерж-этап конвейера сопровождения `ivanarama/PuT`. Запуск headless:
...
1. Очередь: получи **все** открытые PR пагинированным REST, затем локально
   ...
   ```
   gh api --paginate "repos/ivanarama/PuT/pulls?state=open&per_page=100" \
   ...
   весь MERGE. Если его `to` уже получил каноничный proof `reviewed`, обрабатывай
   владельца раньше обычной очереди: proof передаёт ход MERGE, но не освобождает
   барьер; его освобождает только merge/закрытие владельца. При нескольких
   legacy re-ship выбери только
   минимальный номер; остальные сохраняют `ship`, но ждут. Нельзя заранее
   обновлять или мержить следующий PR: это изменит `main` и обесценит аудит
   владельца барьера.
   ...
   ```
   gh api repos/ivanarama/PuT/issues/<N> --jq '[.labels[].name]'
   ```
   ...
   ```
     gh api repos/ivanarama/PuT/pulls/<N> --jq '{sha:.head.sha,state,baseRefName:.base.ref}'
   gh api --paginate "repos/ivanarama/PuT/issues/<N>/comments?per_page=100" \
     --jq '.[] | {id,node_id,created_at,updated_at,author:.user.login,body}'
   gh api --paginate "repos/ivanarama/PuT/issues/<N>/timeline?per_page=100" \
   ...
   **последний переход именно метки `ship`** (`LabeledEvent` или
   ...
   события всех actors. Он обязан быть `LabeledEvent` от `ivanarama`. Старый trusted `LabeledEvent`, после
   ...
   intent — ровно один `PullRequestCommit` без force-push/delete/restore/base-
   change; commit `to` имеет ровно двух родителей `[from, base]`, а `base` —
   предок текущего `main`. Проверяй parents через
   `repos/ivanarama/PuT/commits/<to>` и адресуй intent/done по node id в обоих
   полных GraphQL snapshot. Последний переход `ship` во всей carry-цепочке
   обязан оставаться исходным trusted `LabeledEvent`: снятие/повторная
   ...
   же транзакции. Никакие update/push/merge до успешного SHA+authorization-гейта
   недопустимы.
...
3. По состоянию:
   - **BEHIND** → после полного label+SHA+authorization-гейта, включая допустимое
     ...
     echo '{"expected_head_sha":"<проверенный SHA>"}' | \
       gh api -X PUT repos/ivanarama/PuT/pulls/<N>/update-branch --input -
     ```
     ...
     Если HEAD другой, восстанови самый ранний intent: допустим только новый
     merge-коммит с parents `[from, base]` и ровно один соответствующий
     ...
     `[from, base]` публикует отсутствующий done. После done метку `ship` **не
     снимай**; прекрати **весь запуск MERGE** со статусом «ожидает интеграционное REVIEW».
     REVIEW проверит новый HEAD, а MERGE продолжит без второго клика человека.
   ...
   - **DIRTY (конфликт)** → чинить в отдельном worktree, привязанном к SHA
     ...
     `AGENTS.md`, YAML и DSL нельзя разрешать выбором одной стороны без разбора;
     после конфликта в `.yaml` или `.os` обязательно выполни
     `onebase check --project .` и `onebase lint --project .` (автомёрж режет
     по границам конфликта, не понимая синтаксиса). Содержательный конфликт
     (логика с обеих сторон,
     ...
     правилам, что для BEHIND, затем повтори гейт. REST-сверка перед push не атомарна, поэтому используй
     ...
     Подтверждённый push меняет HEAD: проверь parents `[from, base]`, единственный
     ...
     **весь запуск MERGE**, сохранив `ship`. Ждать CI и мержить новый SHA без
     интеграционного REVIEW нельзя; второй человеческий `ship` не нужен.
   - **CLEAN + требуемые проверки зелёные** → мерж (п. 5).

4. Ожидание CI: цикл «`sleep 120` → перечитать статус», не дольше **35 минут**
   на PR. Требуй успеха всех проверок, которые GitHub считает обязательными для
   `main`, и workflow `Validation & Smoke Tests` из `.github/workflows/ci.yml`.
   Если набор required checks изменился, доверяй branch protection и текущему
   `statusCheckRollup`, а не сохранённому в процедуре списку.
   Красная требуемая проверка → `gh run view <id> --log-failed`, диагноз:
   - флейк-профиль (таймаут, обрыв раннера, сеть) →
...
5. Мерж: выполни последний полный гейт из п. 1 и сохрани GraphQL `node_id`
   ...
   echo '{"merge_method":"merge","sha":"<проверенный SHA>"}' | \
     gh api -X PUT repos/ivanarama/PuT/pulls/<N>/merge --input -
   ```
   ...
   **Сразу после мержа сними `in-work` с закрытых заявок этого PR.** Метку
   ...
   ```
   gh api -X DELETE repos/ivanarama/PuT/issues/<N>/labels/in-work
   ```

   Метки нет — ответ 404, это не ошибка: заявка её и не носила.

   Если тело содержит точные соседние строки `Plan-Issue: #<N>` и
   `Plan-Path: Plans/<file>.md`, перечитай issue и потребуй `state=open`,
   `approved`, `plan-in-review`. Опубликуй комментарий с точной строкой
   `<!-- pp:plan-ready issue=<N> pr=<PR> path=<path> -->`, проверь его через
   `@base64` байт-в-байт, добавь `ready-fix`, затем сними `plan-in-review` и
   `needs-decision`. `approved`, `decision:*` и `queue:p*` сохрани.
...
8. Финал — сводка (что влито, что и почему отложено) и строка:
   ...
   `ИТОГ: ПУСТО (<почему сейчас нет допустимого действия>)`.

   `ГОТОВО` означает, что запуск совершил хотя бы одну durable-мутацию в
   GitHub: merge, push/base-sync, protocol comment или допустимое изменение
   меток. Если запуск только обнаружил, что владелец single-flight ждёт REVIEW,
   и не изменил GitHub, это `ПУСТО`, а не `ГОТОВО`. Не маскируй ожидание под
   продуктивный прогон: PromptPilot использует итог для статистики и решения о
   временной частоте.
