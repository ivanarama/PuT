...
1. Первичный список получай целиком пагинированным REST — жёсткого cap быть не
   ...
   Кандидатов с незавершённым доказанным base-sync или legacy re-ship ставь
   **перед** обычной очередью и сортируй их по номеру. Если такой кандидат есть,
   выбери только самый первый: его интеграционное REVIEW — единственный аудит
   этого запуска, остальные слоты не заполняй. Пока MERGE не вольёт владельца
   single-flight-барьера, нельзя заранее ревьюить следующий PR: первый merge
   снова изменит `main` и обесценит второй аудит. Только когда интеграционных
   кандидатов нет, применяй `(review-depth ASC, number ASC)`. Обычный `ship` без
