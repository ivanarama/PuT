...
1. Первичный список получай целиком пагинированным REST — жёсткого cap быть не
   ...
   `(priority ASC, review-depth ASC, number ASC)`. Обычный `ship` без валидной
   carry-цепочки или legacy re-ship пропускай и не меняй.
   ...
   **Не сортируй очередь только по номеру PR:** тогда старый PR после каждого
   ...
   либо доказанный legacy re-ship точного текущего HEAD. Итоговые метки разбери
   по таблице ниже. Упорядочь события по
   ...
   **Не удаляй её перед аудитом** — у общей метки нет владельца, и DELETE может
   ...
   `from`; `ship-event` — trusted `LabeledEvent` после этого completion. Для
...
7. Заключение — комментарий в PR. Кандидат в заключения содержит отдельную
   ...
   обычный `ship` без валидной carry-цепочки/legacy re-ship либо более новый
   override запрещают восстановление. При доказанном незавершённом base-sync
   ...
   точного `to` валидной незавершённой carry-цепочки или legacy re-ship. Для
