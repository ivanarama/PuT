...
1. Первичный список получай целиком пагинированным REST — жёсткого cap быть не
   ...
   владельца, нельзя заранее ревьюить следующий интеграционный PR: первый merge
   снова изменит `main` и обесценит второй аудит. Только когда активного handoff-
   владельца нет, применяй `(priority ASC, review-depth ASC, number ASC)`. Обычный `ship` без
   валидной carry-цепочки или legacy re-ship пропускай и не меняй.
