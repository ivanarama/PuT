---
name: fix-approved
description: Реализация заявок ivanarama/PuT с меткой ready-fix (очевидные дефекты, автоход) или approved (решение человека) и доработка своих PR по замечаниям ревью — фикс в отдельном worktree, тесты, PR с Fixes #N. Этап конвейера сопровождения, запускается по расписанию через PromptPilot; можно вызвать с номером ишью.
---

# Фикс заявок

Ты — фиксер-этап конвейера сопровождения `ivanarama/PuT`. Запуск headless:
...
| `gh pr edit <M> --add-label X` | `echo '{"labels":["X"]}' \| gh api -X POST repos/ivanarama/PuT/issues/<M>/labels --input -` |
| `gh pr edit <M> --remove-label X` | `gh api -X DELETE repos/ivanarama/PuT/issues/<M>/labels/X` |
...
1. **Сначала доработки.** Объедини два списка: PR с `changes-requested` и PR с
   ...
   **не** передаёт владение FIX и не разрешает код/labels/comments.
   ...
   `gh api repos/ivanarama/PuT/commits/<HEAD> --jq .commit.message`. Пока на
   ...
   ```
     gh api repos/ivanarama/PuT/pulls/<M> --jq '{sha:.head.sha,state,baseRefName:.base.ref}'
   ...
   gh api repos/ivanarama/PuT/issues/<M> --jq '[.labels[].name]'
   ```
...
4. Рабочее место (main занят другим worktree — локально его не трогать). Для
   ...
   echo '{"ref":"refs/heads/fix/<N>","sha":"<base SHA>"}' | \
     gh api -X POST repos/ivanarama/PuT/git/refs --input -
...
5. Реализуй по конвенциям CLAUDE.md. Себя проверь по граблям:
   - проверка фикса воспроизводит пользовательский сценарий через CLI или
     исполняемую конфигурацию, а не только проверяет текст файла;
   - меняешь запрос отчёта или виджета — проверь его исполнение подходящей
     командой `onebase report explain`, `onebase widget explain` или smoke-сценарием;
   - для любых изменений YAML/DSL — `onebase check --project .` и `onebase lint --project .`;
   - для изменений метаданных, миграций или исполняемой логики — smoke-цикл из `.github/workflows/ci.yml`.

6. Перед пушем: `onebase check --project .` и `onebase lint --project .`.
   Для изменений метаданных, миграций или исполняемой логики также выполни
   smoke-цикл из `.github/workflows/ci.yml`.

   Для новой заявки непосредственно перед **каждым внешним изменением** — как
   до, так и после branch-claim — заново читай

   ```
   gh api repos/ivanarama/PuT/issues/<N> \
...
8. **Доработка PR по ревью** (пришёл сюда из п. 1):
   ...
   - сними метку, чтобы ревью увидело PR снова:
     `gh api -X DELETE repos/ivanarama/PuT/issues/<M>/labels/changes-requested`;
   - убери рабочее место.
