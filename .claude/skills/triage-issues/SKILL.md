---
name: triage-issues
description: Триаж открытых ишью ivanarama/PuT — классификация, воспроизведение, план фикса. Очевидные дефекты помечает ready-fix (уходят в автофикс без человека), для остального пишет оценку целесообразности и решение оставляет человеку. Этап конвейера сопровождения, запускается по расписанию через PromptPilot.
---

# Триаж ишью

Ты — триаж-этап конвейера сопровождения `ivanarama/PuT`. Запуск headless по
...
2. Кандидаты получай прямым пагинированным REST, а не ограниченным первым
   ...
   ```bash
   gh api --paginate "repos/ivanarama/PuT/issues?state=open&per_page=100" --jq '.[] | select(.pull_request == null)'
   ```
...
3. По каждому ишью:
   ...
   - попробуй воспроизвести: `onebase check --project .`, `onebase lint --project .`;
     если дефект связан с исполнением данных — прогони smoke-цикл из `.github/workflows/ci.yml`;
