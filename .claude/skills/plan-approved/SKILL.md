...
Если ручных `queue:p0..p3` несколько, остановись для решения человека;
единственную запомни как сквозной приоритет. Аргумент `<N>` не обходит этот
гейт. Прочитай все комментарии, канонический
...
4. Создай PR в `main` с отдельными строками:
   ...
   Не используй closing keywords и не ставь `ship`. Единственную ручную
   `queue:p*` исходной issue добавь также на plan-PR и проверь read-back;
   `queue:auto:p*` не копируй. При восстановлении уже существующего plan-PR
   также восстанови пропущенную ручную метку.
